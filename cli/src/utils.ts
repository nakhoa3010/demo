...
export async function isOraklNetworkApiHealthy() {
  try {
    return 200 === (await axios.get(API_URL))?.status
  } catch (e) {
    console.error(`Orakl Network API [${API_URL}] is down`)
    return false
  }
}
...
export async function isOraklDelegatorHealthy() {
  try {
    return 200 === (await axios.get(DELEGATOR_URL))?.status
  } catch (e) {
    console.error(`Orakl Network Delegator [${DELEGATOR_URL}] is down`)
    return false
  }
}
