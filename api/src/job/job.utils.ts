...
import { FETCH_TIMEOUT, PROVIDER_URL } from '../app.settings'
import { REDUCER_MAPPING, buildReducer, checkDataFormat, pipe } from '../utils'
...
export async function extractUniswapPrice(adapter, decimals) {
  const provider = await providerByChain()
...
export async function providerByChain() {
