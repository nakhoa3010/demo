...
import { getOperatorAddress, getReporterByAddress } from '../apis'
import {
...
import { buildHeartbeatJobId, buildSubmissionRoundJobId, convertBigIntToString } from '../utils'
...
import { AggregatorAbi as aggregatorAbis } from '../constants/agregator.abi'
 ...
 * aggregated data from the Network API for a specific
...
export function aggregatorJob(
  ...
  const iface = new Interface(aggregatorAbis)
  ...
  async function wrapper(job: Job) {
    ...
    try {
      ...
      logger.debug(
        { aggregatorId, fetchedAt: timestamp, submission: Number(submission) },
...
async function sendTx(tx: any, logger: Logger) {
  ...
  const txParams = {
    ...
    to: tx.to,
    payload: tx.payload,
    gasLimit: DATA_FEED_FULFILL_GAS_MINIMUM,
    logger
  }
...
function heartbeatJob(aggregatorQueue: Queue, state: State, _logger: Logger) {
  ...
  async function wrapper(job: Job) {
    ...
    try {
      ...
      const operatorAddress = await getOperatorAddress({ aggregatorAddress: oracleAddress, logger })
      ...
      logger.debug(
        { oracleRoundState: convertBigIntToString(oracleRoundState) },
        'oracleRoundState'
      )
      ...
      if (eligibleToSubmit) {
        ...
        // a deadlock and the  Network Reporter service failed on
        ...
        throw new XOracleError(XOracleErrorCode.NonEligibleToSubmit, msg)
      }
    } catch (e) {
 ...
 * @param {Logger} pino logger
 * @return {void}
 * @except {RivalzErrorCode.UnexpectedNumberOfDeadlockJobs} raise when
...
async function removeAggregatorDeadlock(aggregatorQueue: Queue, jobId: string, logger: Logger) {
  ...
  if (blockingJob.length == 1) {
    ...
    throw new XOracleError(
      XOracleErrorCode.UnexpectedNumberOfDeadlockJobs,
...
export function deviationJob(reporterQueue: QueueType, _logger: Logger) {
  ...
  async function wrapper(job: Job) {
    ...
    logger.debug({ ...inData, submission: Number(inData.submission) }, 'inData')
    ...
    try {
      ...
      logger.debug(
        { aggregatorHash, fetchedAt: timestamp, submission: Number(submission) },
        'Latest data aggregate'
      )
